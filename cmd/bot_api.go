...
func registerHTTPAPI(mux *http.ServeMux, bot *tele.Bot, creds *config.Credentials) {
	...
	mux.HandleFunc("/inject/message", func(w http.ResponseWriter, r *http.Request) {
		...
		logger.Info(fmt.Sprintf("Inject message API: target=%s text=%s image=%s", tmuxStr, truncateStr(req.Text, 200), req.ImagePath))
		if err := injectMessage(bot, tmuxStr, req.Text, req.ImagePath); err != nil {
	...
	mux.HandleFunc("/session/log", func(w http.ResponseWriter, r *http.Request) {
		...
		if jsonlPath == "" {
			home, _ := os.UserHomeDir()
			jsonlPath = filepath.Join(home, ".claude", "projects", projectSlug(info.cwd), sessionID+".jsonl")
		}
		...
		for _, jf := range jsonlFiles {
			...
			var fileEntries []transcriptLogEntry
			if info.backend == "codex" {
				fileEntries = parseCodexTranscript(f, noTools, filteredTools, formatToolDetail)
			} else {
				fileEntries = parseCCTranscript(f, noTools, filteredTools, formatToolDetail)
			}
			f.Close()
