...
curl -s "http://127.0.0.1:$TEST_PORT/session/name?session_id=$SESSION_ID&name=$AGENT_NAME" > /dev/null
...
# Start merge mode
ENCODED_PANE=$(printf '%s' "$CLAUDE_PANE" | jq -sRr @uri)
