...
while [ $ELAPSED -lt $TIMEOUT ]; do
  ...
  if [ "$LOG_AFTER" -gt "$LOG_BEFORE_HELLO" ]; then
    if tail -n +"$((LOG_BEFORE_HELLO + 1))" "$LOG_FILE" | grep -E "Stream send|Stream relabel ✅:|: Stop \[|Stop terminal: outcome=direct_send" > /dev/null 2>&1; then
      FOUND=true
      break
    fi
  fi
...
if [ "$FOUND" = true ]; then
  pass "1st TG streamed notification sent (hook → bot → TG, Stream send or Stop-delivery)"
else
  fail "1st TG streamed notification (no Stream send or Stop-delivery within ${TIMEOUT}s)"
fi
